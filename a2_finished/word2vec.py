...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    length = centerWordVec.shape[0]
    numerator = softmax(np.dot(outsideVectors, centerWordVec))
    denominator = np.sum(softmax(np.dot(outsideVectors, centerWordVec)), axis=0)
    y_hat = numerator / denominator # shape: (num words in vocab, )
    loss = - np.log(y_hat[outsideWordIdx])
    gradCenterVec = np.dot(outsideVectors.T, y_hat) - outsideVectors[outsideWordIdx]
...
def negSamplingLossAndGradient(
    ...
    outsideVectors_idx = outsideVectors[indices]  # extract indices-th vectors from outsideVectors
    u_o = outsideVectors_idx[0]
    u_w = outsideVectors_idx[1:] #(num of indices, word vector length)
    loss = - np.log(sigmoid(u_o @ centerWordVec)) - np.sum(np.log(sigmoid(- u_w @ centerWordVec)), axis=0)
    # print("loss:", loss)
    gradCenterVec = (sigmoid(u_o @ centerWordVec) - 1) * u_o.T + np.dot(u_w.T, sigmoid(u_w @ centerWordVec))
    gradOutsideVecs= np.zeros((outsideVectors.shape[0], outsideVectors.shape[1]))
    gradOutsideVecs[indices[0]] = (sigmoid(u_o @ centerWordVec) - 1) * centerWordVec.T
    gradu_w = np.outer(sigmoid(u_w @ centerWordVec), centerWordVec.T)
    assert gradu_w.shape[0] == K
    for i in range(K):
        idx = negSampleWordIndices[i]
        gradOutsideVecs[idx] += gradu_w[i]
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    curCenterIdx = word2Ind[currentCenterWord]
    centerWordVec = centerWordVectors[curCenterIdx]
    # outsideWordIndices = [word2Ind[outsideWord] for outsideWord in outsideWords]
    # assert len(outsideWordIndices) == windowSize
    for outsideWord in outsideWords:
        outsideWordIdx = word2Ind[outsideWord]
        cur_loss, cur_gradCenterVecs, cur_gradOutsideVectors = word2vecLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset)
        loss += cur_loss
        gradCenterVecs[curCenterIdx] += cur_gradCenterVecs
