...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        w1 = torch.empty(self.embed_size * n_features, self.hidden_size)
        self.embed_to_hidden_weight = nn.Parameter(nn.init.xavier_uniform_(w1), requires_grad=True)

        b1 = torch.empty(1, self.hidden_size)
        self.embed_to_hidden_bias = nn.Parameter(nn.init.uniform_(b1), requires_grad=True)

        self.dropout = nn.Dropout(p = self.dropout_prob)

        w2 = torch.empty(self.hidden_size, self.n_classes)
        self.hidden_to_logits_weight = nn.Parameter(nn.init.xavier_uniform_(w2), requires_grad=True)

        b2 = torch.empty(1, self.n_classes)
        self.hidden_to_logits_bias = nn.Parameter(nn.init.uniform_(b2), requires_grad=True)
